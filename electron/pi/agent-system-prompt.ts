...
function buildL0ProjectContext(core: ProjectCoreData | null, language: WritingLanguage): string | null {
  ...
  parts.push(writingLanguageText(
    ...
    [
      ...
      '- 章节正文撰写、重写与局部润色（Draft Management）：当作者要求撰写某章正文、整章推倒重写或对正文进行局部润色时，请使用 `manage_drafts` 工具。撰写/重写可使用 `action: "write"`，局部润色修改时可先读取正文再调用 `action: "replace_excerpt"` 精准替换，严禁凭记忆猜测原文。',
    ...
    [
      ...
      '- Chapter Draft Writing & Modification (Draft Management): When writing new chapter drafts, rewriting entire chapters, or polishing specific passages, call the `manage_drafts` tool. Use `action: "write"` for initial creation or new version rewrite, and `action: "replace_excerpt"` for precise excerpt replacement.',
