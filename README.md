...
| 写作助手 | 右侧助手分「项目助手」与「界面助手」：项目助手跟着当前项目走，能读项目资料、发起工作流，还能在项目目录内读写文件、执行命令（每次都要你确认），会话存在项目目录内；界面助手不依赖项目，随时可用，工作目录是自己的 `~/.vela/workspace`，会话存在应用数据目录。打开项目自动切到项目助手。技能按需读取：目录里只有名字与描述，助手需要时自己取正文。 |
